...
const FruitNinja: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      socket = new WebSocket('ws://localhost:8080'); // Room Manager
      socket.onopen = () => {
        // eslint-disable-next-line no-console
        console.log('✅ Connected to Room Manager');
        socket?.send(JSON.stringify({ type: 'join', room: 'room123', id: 'FrontendClient' }));
      };
      ...
      socket.onerror = () => {
        // optional: add error handling UI/toast
      };
      setWs(socket);
    } catch {
      // optional: add connection failure handling
    }
  ...
  const handleCanvasClick = useCallback(
    (event: { clientX: number; clientY: number }) => {
      ...
      if (checkCollision(mouseX, mouseY, fruit, GAME_CONFIG.FRUIT_SIZE)) {
        ...
        // Animate sword
        const sword = (gameStateRef.current.swords as any)[`player${player}`];
        sword.swinging = true;
        ...
        // Send score to server if connected
    ...
    [gameActive, score1, score2, spawnFruit, ws]
  );
  ...
  const gameLoop = useCallback(
    (currentTime: number) => {
      ...
      if (ctx && gameStateRef.current.currentFruit) {
      ...
      if (ctx && gameStateRef.current.slashEffect) {
  ...
  // Drawing functions (kept identical in behavior)
  ...
  const drawSwords = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.fillRect(
      -GAME_CONFIG.SWORD_WIDTH / 2,
      -GAME_CONFIG.SWORD_HEIGHT / 2,
      GAME_CONFIG.SWORD_WIDTH,
      GAME_CONFIG.SWORD_HEIGHT
    );
  ...
  const drawFruit = (ctx: CanvasRenderingContext2D, fruit: any) => {
    ...
    const fruitInfo = (fruitData as any)[fruit.type] || fruitData.apple;
  ...
  const drawSlashEffect = (ctx: CanvasRenderingContext2D, slash: any) => {
  ...
  const drawUI = (ctx: CanvasRenderingContext2D) => {
    ...
    ctx.font = '20px Arial';
    ctx.fillText(
      `Round ${round} | First to ${GAME_CONFIG.WINNING_SCORE} wins!`,
